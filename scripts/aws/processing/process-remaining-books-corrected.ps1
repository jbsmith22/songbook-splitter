# Upload remaining books to S3 and process them with CORRECT input format
...
Write-Host "Upload and Process Remaining Books (CORRECTED)" -ForegroundColor Cyan
Write-Host "================================================================================" -ForegroundColor Cyan
Write-Host ""
...
foreach ($book in $booksToProcess) {
    ...
    $executionName = "corrected-$sanitizedName-$timestamp"
...
if ($executionArns.Count -gt 0) {
    $executionArns | Export-Csv "remaining-books-executions-corrected.csv" -NoTypeInformation -Encoding UTF8
    Write-Host "Execution details saved to: remaining-books-executions-corrected.csv" -ForegroundColor Cyan
    Write-Host ""
