"""
Execute reconciled file-level reconciliation decisions
...
# Load reconciled decisions
print("Loading reconciliation_decisions_2026-02-02_reconciled_v2.json...")
decisions_file = Path(__file__).parent / 'reconciliation_decisions_2026-02-02_reconciled_v2.json'
with open(decisions_file, 'r', encoding='utf-8') as f:
    decisions_root = json.load(f)

decisions = decisions_root['decisions']
print(f"Loaded {len(decisions)} folders with decisions")
print(f"Total operations: {sum(len(fd.get('fileDecisions', {})) for fd in decisions.values())}\n")
...
print(f"Executing reconciled file decisions...\n")
...
for folder_path, folder_decisions in decisions.items():
    ...
    for filename, file_decision in file_decisions.items():
        ...
        # Get correct paths from file decision (has bracket notation for S3)
        file_local_path = file_decision.get('local_path', local_base)
        file_s3_path = file_decision.get('s3_path', s3_base)

        if action == 'rename-s3':
            # Rename file in S3
            normalized_name = file_decision.get('normalized_name')
            if normalized_name:
                from_path = f"{file_s3_path}/{filename}"
                to_path = f"{file_s3_path}/{normalized_name}"
            ...
            normalized_name = file_decision.get('normalized_name')
            if normalized_name:
                from_path = f"SheetMusicIndividualSheets/{file_local_path}/{filename}"
                to_path = f"SheetMusicIndividualSheets/{file_local_path}/{normalized_name}"
                rename_local_file(from_path, to_path)

        elif action in ['copy-to-local', 'copy-s3-to-local']:
            # Copy from S3 to local
            s3_path = f"{file_s3_path}/{filename}"
            local_path = f"SheetMusicIndividualSheets/{file_local_path}/{filename}"
